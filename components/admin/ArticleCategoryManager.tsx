...
import { createArticleCategory, updateArticleCategory, deleteArticleCategory } from "@/lib/actions/article-categories"
...
export function ArticleCategoryManager({ categories }: { categories: ArticleCategory[] }) {
  ...
  async function handleAdd() {
    ...
    setSaving(true)
    try {
      await createArticleCategory({ name, slug: slug || generateSlug(name), description: description || undefined, sortOrder })
      setAdding(false)
      setName("")
      setSlug("")
      setDescription("")
  ...
  async function handleUpdate() {
    ...
    setSaving(true)
    try {
      await updateArticleCategory(editing, { name, slug, description: description || undefined, sortOrder })
      setEditing(null)
      router.refresh()
    } finally {
      setSaving(false)
      ...
      <table className="w-full text-sm">
        <thead className="bg-muted/50">
          <tr>
            ...
            <th className="text-left px-4 py-3 font-medium">Статей</th>
            ...
            <tr key={c.id} className="border-t border-border">
              {editing === c.id ? (
                <>
                  ...
                  <td className="px-4 py-2">
                    <input value={description} onChange={(e) => setDescription(e.target.value)} className="w-full h-8 px-2 rounded border border-input text-sm" />
                  </td>
                  <td className="px-4 py-2">
                    <input type="number" value={sortOrder} onChange={(e) => setSortOrder(Number(e.target.value))} className="w-16 h-8 px-2 rounded border border-input text-sm" />
                  </td>
                  <td className="px-4 py-2">{c.articleCount}</td>
                  <td className="px-4 py-2">
                ...
                <>
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{c.articleCount}</td>
                    ...
                    <div className="flex gap-1">
                      ...
                      {c.articleCount === 0 && (
                        <ConfirmDialog
                          ...
                          message={`Рубрика "${c.name}" будет удалена.`}
                          onConfirm={async () => {
      ...
      <div className="p-4 border-t border-border">
        {adding ? (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <input type="number" value={sortOrder} onChange={(e) => setSortOrder(Number(e.target.value))} placeholder="Порядок" className="w-20 h-8 px-2 rounded border border-input text-sm" />
          ...
          <button onClick={() => setAdding(true)} className="flex items-center gap-2 text-sm text-primary hover:underline">
            <Plus className="w-4 h-4" /> Добавить рубрику
          </button>
        )}
      </div>
    </div>
